...
class StaffPage extends React.Component{
    ...
    reloadData(){
        ...
        // 查询数据，进行数据绑定
        axios.get("/staff/findAllWithDepartment")
        .then((result)=>{
            //将查询更新到state中
            this.setState({
                list:result.data
    ...
    // 7. 添加表单（提前导组件StaffForm 和 在局部状态中加入visible:true）
    // 7.2 取消按钮的事件处理函数
    handleCancel = () => {
    ...
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            // 表单校验完成后与后台通信进行保存
            axios.post("/staff/saveOrUpdate",values)
            .then((result)=>{
                message.success(result.statusText)
    ...
    render(){
        ...
        return (
            ...
            <div className={styles.firstdiv}>
                {/* 1.1.1.标题组 */}
                <div className={styles.title}><h1 align='center'>职工管理</h1></div>
                {/* 1.1.2.按钮组 */}
