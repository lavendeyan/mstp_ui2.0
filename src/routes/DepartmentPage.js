...
class DepartmentPage extends React.Component{
    ...
    handleDelete(id){
        Modal.confirm({
            ...
            content: '数据删除后将无法恢复',
            onOk:()=>{
                axios.get("/department/deleteById",{
                    params:{
                        id:id
                    }
                })
    ...
    handleBatchDelete(){
        Modal.confirm({
            ...
            content: '数据删除后将无法恢复',
            onOk:()=>{
                axios.post("/department/batchDelete",{
                    ids:this.state.ids
                })
                .then((result)=>{
                    //提示删除成功
    ...
    // 7. 添加表单（提前导组件DepartmentForm 和 在局部状态中加入visible:true）
    // 7.2 取消按钮的事件处理函数
    handleCancel = () => {
    ...
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            // 表单校验完成后与后台通信进行保存
            axios.post("/department/saveOrUpdate",values)
            .then((result)=>{
                message.success(result.statusText)
    ...
    render(){
        ...
        return (
            ...
            <div className={styles.firstdiv}>
                {/* 1.1.1.标题组 */}
                <div className={styles.title}><h1 align='center'>科室/部门管理</h1></div>
                {/* 1.1.2.按钮组 */}
